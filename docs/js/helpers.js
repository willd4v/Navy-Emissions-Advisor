...
const buildComponentSpecList = (component) => {
  const componentType = getComponentType(component.typeName, data.componentTypes);
  return (
    `<div class="spec-list display-spec-list">
...
const getComponentType = (typeParamName) => {
  const componentTypeId = Object.keys(data.componentTypes).find(ct => {
    const componentType = data.componentTypes[ct];
    return componentType.parameterName === typeParamName;
  });
  return data.componentTypes[componentTypeId];
}
...
const apiReq = (url, dataName, type, postData) => {
  // Get JWT from cookie
...
const userQuotaCheck = (el, e) => {
  const dataType = e.target.dataset.type;

  const quotaVal = data.quota[dataType];
  const dataName = dataType === "sizing" ? dataType : dataType + "s";

  const numItems = data[`${dataName}`].length;

  if (numItems >= quotaVal) {
    alert(`You have reached user account quota of ${quotaVal} entries in ${dataType}s. To add a new ${dataType}, you will first need to delete one of your existing ${dataType}s.`);
    return;
  }

  $(`#new-${dataType}-modal`).modal("show");
}
